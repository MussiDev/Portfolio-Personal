...
new GLTFLoader().parse(
	...
	(gltf) => {
		...
		// Snaps each editorial anchor (entities/brainAnchors.ts) to the nearest
		// tissue point here, at build time, instead of scanning the ~30k
		// points for each of the 6 anchors on the client on every load.
		...
		// Content-hashed name: lets the .bin be served with immutable caching
		// for a year (next.config.js) without risking serving a stale
		// binary the next time the model changes.
